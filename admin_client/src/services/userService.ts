...
export const userService = {
  ...
  getUserById: (userId: string): Promise<User> =>
    fetchWithAuth<User>(`/gateway/users/${userId}`),
  ...
  getActiveUsers: (params?: Omit<UserQueryParams, "state">) =>
    userService.getUsers({ ...params, state: "active" }),

  // Get locked users (helper method)
  getLockedUsers: (params?: Omit<UserQueryParams, "state">) =>
    userService.getUsers({ ...params, state: "locked" }),

  lockUser: (userId: string) =>
    fetchWithAuth<{ success: boolean }>(`/gateway/users/lock/${userId}`, {
      method: "PATCH",
    }),

  // Unlock user (set state to Active)
  unlockUser: (userId: string) =>
    fetchWithAuth<{ success: boolean }>(`/gateway/users/unlock/${userId}`, {
      method: "PATCH",
    }),
};
