...
export default function ChatApp() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("initiateChatWithUser", handleInitiateChat as EventListener)
    ...
    const checkPendingChatUser = () => {
      ...
      if (pendingChatData) {
        try {
          ...
          console.log("[ChatApp] Found pending chat user in localStorage:", userData)
    ...
    return () => {
      window.removeEventListener("initiateChatWithUser", handleInitiateChat as EventListener)
    }
  }, [])

  // Handle pending chat user
  const handlePendingChatUser = async (userId: string, autoEnterChat = false) => {
    console.log(`[ChatApp] Processing pending chat with user: ${userId}, autoEnter: ${autoEnterChat}`)

    // Wait for groups to load if not loaded yet
    let retryCount = 0
    const maxRetries = 10
    while (!joinedGroups.length && retryCount < maxRetries) {
      console.log(`[ChatApp] Waiting for groups to load... (${retryCount + 1}/${maxRetries})`)
      await new Promise((resolve) => setTimeout(resolve, 500))
      retryCount++
    }

    // Tìm group chat với user này
    let targetGroup = joinedGroups.find((group) => group.users.some((u) => u.id === userId) && group.users.length === 2)

    if (targetGroup) {
      console.log(`[ChatApp] Found existing group for user ${userId}:`, targetGroup)
      const otherUserId = targetGroup.users.find((u) => u.id !== session?.user?.id)?.id
    ...
    console.log(`[ChatApp] No existing group found, searching for user: ${userId}`)
    try {
      ...
      targetGroup = response.items?.find(
        (group) => group.users.some((u) => u.id === userId) && group.users.length === 2,
      )
    ...
    console.log(`[ChatApp] No existing group found, SignalR will create new group for user: ${userId}`)
    ...
    try {
      ...
      const tempGroup: SignalRGroup = {
        ...
        users: [
          { id: session?.user?.id || "", username: session?.user?.name || "", photoUrl: session?.user?.image || "" },
          { id: userId, username: "Loading...", photoUrl: "/placeholder.svg?height=48&width=48" },
        ],
        lastMessage: null,
      ...
      console.log(`[ChatApp] Connecting to SignalR to create new group with user: ${userId}`)
      const connected = await connectToUserForChat(userId)

      if (connected) {
        console.log(`[ChatApp] Successfully connected - new group should be created by backend`)
  ...
  const handleGroupSelect = async (group: SignalRGroup, otherUserId: string | null, autoEnterChat = false) => {
    if (!otherUserId) {
      console.error("Cannot find other user ID")
      return
    }

    console.log(`[ChatApp] Selecting group: ${group.id}, otherUserId: ${otherUserId}, autoEnter: ${autoEnterChat}`)
    ...
    try {
      ...
      console.log(`[ChatApp] Attempting to connect to MessageHub for user: ${otherUserId}`)
      const connected = await connectToUserForChat(otherUserId)

      if (connected) {
        console.log(`[ChatApp] Successfully connected to MessageHub`)
      } else {
        console.warn("Failed to connect to MessageHub, but continuing with chat")
  ...
  const displayGroups = searchQuery.trim() ? searchResults : filterConversationsByTab(joinedGroups)
            ...
            <>
              {displayGroups.map((group) => (
                <ConversationItem
                  ...
                  onClick={(otherUserId) => handleGroupSelect(group, otherUserId, false)} // false = không auto enter
